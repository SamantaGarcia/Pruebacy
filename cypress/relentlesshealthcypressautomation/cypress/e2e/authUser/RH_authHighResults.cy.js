describe('High Results - Relentless Health Website', () => {
    ...
    beforeEach(() => {
        ...
        cy.get('.MuiTypography-root.MuiTypography-body1.css-rr7ocq').should('contain', 'NASEM level high');
        cy.get('.MuiTypography-root.MuiTypography-body1.css-k01lgd') 
            .invoke('text') 
            .then((text) => {
                nasemLevel = parseFloat(text.trim());  
                expect(nasemLevel).to.be.greaterThan(20);
            });
    });
    ...
    it('Card View', function() {
        const highResults  = this.pfasResults.highResults;
        ...
        Object.entries(highResults).forEach(([key, value]) => {
        ...
        cy.get('.recharts-layer.recharts-scatter-symbol')
            .each(($elemento, index) => { 
                ...
                cy.wrap($elemento)
                    ...
                    .invoke('text')
                    .then((cardText) => {
                        // Verify if at leats 1 of the PFAS from highResults is in the card
                        cy.log(`Card text: ${cardText.trim()}`);
                        const matchFound = Object.entries(highResults).some(([key, value]) => {  
                        ...
                        expect(matchFound, 'The PFAS info from highResults is in the card').to.be.true;
                    });
            });
    });
