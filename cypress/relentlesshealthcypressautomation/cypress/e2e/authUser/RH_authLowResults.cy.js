describe('Low Results - Relentless Health Website', () => {
    ...
    function clickUntilTextAppears() {
        cy.get('button')
          ...
          .wait(500)
          .then(() => {
            cy.get('.MuiTypography-root.MuiTypography-body1.css-98j2le')
              .then(($el) => {
                if ($el.text().includes('NASEM level low')) {
                  cy.log('Report found');
                  cy.get('button').find('img[src="/icons/back-white-arrow.svg"]').parent()
                    .click() //one more click
                  return;
                } else {
    ...
    beforeEach(() => {
        ...
        cy.get('.MuiBox-root.css-1qptri0') 
            .invoke('text') 
            .then((text) => {
                nasemLevel = parseFloat(text.trim()); //obtain nasem level
                expect(nasemLevel).to.be.lessThan(2);
            });
    });
    ...
    it('Card View', function() {
        const lowResults  = this.pfasResults.lowResults;
        ...
        Object.entries(lowResults).forEach(([key, value]) => {
        ...
        cy.get('.recharts-layer.recharts-scatter-symbol')
            .each(($elemento, index) => { 
                ...
                cy.wrap($elemento)
                    ...
                    .invoke('text')
                    .then((cardText) => {
                        // Verify if at leats 1 of the PFAS from lowResults is in the card
                        cy.log(`Card text: ${cardText.trim()}`);
                        const matchFound = Object.entries(lowResults).some(([key, value]) => {  
                        ...
                        expect(matchFound, 'The PFAS info from lowResults is in the card').to.be.true;
                    });
            });
    });
});
